...
SetSysEvent('banner_type_list');//添加日志功能
...
function banner_type_del(id){
	$.post("action/banner_type_ajax.php",{id:id,action:'del'},function(data){
		if(data){
			if(data=="yes"){
				mx_tipmsg('亲，删除成功！');
...
function edit_paixu(id){
	...
	if(paixu){
		$.post("action/banner_type_ajax.php",{id:id,sort_id:paixu,action:'sort'},function(data){
			if(data){
				if(data=="yes"){
					mx_tipmsg('亲，修改成功！');
					}else{
...
//修改图文分类名称
function edit_banner_type_name(obj,id){
	var title = obj.value;
	if(id){
		$.post("action/banner_type_ajax.php",{id:id,title:title,action:'title'},function(data){
			if(data){
				if(data=="yes"){
					mx_tipmsg('亲，修改成功！');
					}else{
...
<table width="98%" align="center" cellpadding="3" cellspacing="1" style="margin:0 auto">
  <tr>
    <td><input type="button" class="button list_add" value="添加类别" onClick="location.href='banner_type_add.php?type=<?php echo $type?>'"></td>
  </tr>
  <tr>
...
<tr id="type_list_<?php echo $rows['c_id']?>" class="list_tr">
  ...
  <td align="center" class="typecon"><input type="text" class="input_title" onChange="edit_banner_type_name(this,<?php echo $rows['c_id']?>)" value="<?php echo $rows['c_title']?>"></td>
  <td width="6%" align="center" class="typecon"><table width="100%" cellpadding="0" cellspacing="0">
      <tr align="center" class="type_op">
        <td><span class="mxicon"><a title="修改" href="banner_type_edit.php?id=<?php echo $rows['c_id']?>">&#xe905;</a></span></td>
        <td style="position:relative"><span class="mxicon"><a onClick="type_del_op(this,'<?php echo $rows['c_id']?>',5,0)" title="删除">&#xe9ac;</a></span></td>
      </tr>
    </table></td>
</tr>
<?php

...
$sql_num = "select lc_id from ".lc()."_news where lc_del=0 and lc_zt=1 and lc_phone=1 ";
if($cid<>0){
...
$sql = "select * from ".lc()."_news where lc_del=0 and lc_zt=1 and lc_phone=1 ";
if($cid <> 0){
...
if($rs){
	while($rows = mysql_fetch_assoc($rs)){
		...
		<li>
		<div>
		<h3><a href="news_show.php?id=<?php echo $rows["lc_id"]?>" title="<?php echo $rows["lc_title"]?>"><?php echo $rows['lc_title']?></a></h3>
		<p class="con"><?php echo mb_strcut(strip_tags(str_replace("../","",$rows['lc_phone_content'])),0,100,"utf-8")?></p>
